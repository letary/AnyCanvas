...
struct TgfxHooks {
  ...
  // The advance width of one line of text in that font (the host's measurer).
  std::function<float(const std::string& text, const tgfx::Font&)> measure;
  // Draw one line with its baseline origin at (x, y) — the host's shaper / glyph run drawer.
  std::function<void(tgfx::Canvas*, const std::string& text, float x, float y, const tgfx::Font&, const tgfx::Paint&)> drawLine;
...
// save stack (the painter resets both). Clearing the surface is the caller's decision.
